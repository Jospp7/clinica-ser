...
const WhatsAppButton = () => {
  ...
  return (
    <div className="wa-float-wrap">
      <button onClick={handleClick} aria-label={`Contactar por WhatsApp · ${SITE.whatsappHorario}`} title={`WhatsApp · ${SITE.whatsappHorario}`} className="wa-float-btn">
      ...
      <span className="wa-float-tooltip">WhatsApp · {SITE.whatsappHorario}</span>
      ...
      <style>{`
        ...
        @media (prefers-reduced-motion: no-preference) {
          ...
          @keyframes waPulse { 0%, 100% { transform: scale(1); opacity: 0.6; } 60% { transform: scale(1.55); opacity: 0; } }
        }
      `}</style>
    </div>
  );
