...
const Guias = () => {
  ...
  return (
    <main>
      ...
      <style>{`
        .guias-idx__hero { background: #1B2A4A; padding: clamp(120px,14vw,180px) 24px clamp(56px,7vw,90px); text-align: center; }
        .guias-idx__hero-inner { max-width: 780px; margin: 0 auto; }
        .guias-idx__tag { display: inline-block; font-family: 'Inter', sans-serif; font-size: 11px; font-weight: 700; letter-spacing: .14em; color: #C8E64A; margin-bottom: 14px; }
        .guias-idx__title { font-family: 'Playfair Display', Georgia, serif; font-size: clamp(32px,5vw,52px); color: #fff; margin: 0 0 14px; }
        .guias-idx__sub { font-family: 'Inter', sans-serif; font-size: 16px; color: rgba(255,255,255,0.85); margin: 0; }
        ...
        .guias-idx__card-icon { display: inline-flex; align-items: center; justify-content: center; width: 52px; height: 52px; border-radius: 14px; background: #F2F6E0; color: #1B2A4A; margin-bottom: 18px; }
        .guias-idx__card-tag { font-family: 'Inter', sans-serif; font-size: 10px; font-weight: 700; letter-spacing: .14em; color: #8AB83A; margin-bottom: 8px; }
        .guias-idx__card-title { font-family: 'Playfair Display', Georgia, serif; font-size: 24px; color: #1B2A4A; margin: 0 0 10px; }
        .guias-idx__card-text { font-family: 'Inter', sans-serif; font-size: 15px; line-height: 1.6; color: #444; margin: 0 0 20px; }
        .guias-idx__card-cta { margin-top: auto; display: inline-flex; align-items: center; gap: 8px; font-family: 'Inter', sans-serif; font-size: 14px; font-weight: 700; color: #1B2A4A; }
      `}</style>
    </main>
  );
};
